...
nodeArvore *BuildHuffmanTree(int *listaBytes)
{
    ...
    while (l.elementos > 1) 
    {
        ...
        printf("\n");
        printf("first Pop na lista neste elemento: %c, freq: %d",nodeEsquerdo->c,nodeEsquerdo->frequencia);
       
        nodeArvore *nodeDireito = popMinLista(&l);
        printf("\n");
        printf("SECOND Pop na lista neste elemento: %c, freq: %d",nodeDireito->c,nodeDireito->frequencia);
